...
class EntriesController < ApplicationController
  ...
  before_action :set_entry, only: %i[show edit update destroy finalize play]
  ...
  def finalize
    ...
    respond_to do |format|
      if @entry.update({ finished: is_final })
        ...
        logger.warn @entry.errors.full_messages.to_s
  ...
  def entry_params
    params.require(:entry).permit(:word, :sampa, :comment, :user_id, :dictionary_id,
                                  :warning, :finished, :pos, :lang, :is_compound, :comp_part,
                                  :prefix, :dialect)
  end
